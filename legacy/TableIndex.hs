module SlowQL.TableIndex where
    ...
    createIndex table name keys=do
        ...
        chunk<-newBitArray (0, FS.pageSize-1) False
    ...
    compactChunk chunk=do
        ...
        schunk<-freezeBitArray $ bitsetContent chunk
        putStrLn $ show $ length $ bits schunk
        return $ map LR.buildWord8 (LR.chunksOf' 8 $ bits schunk)
    ...
    writeBTree TableIndex {file=file} (BTreeLeaf index value prevLeaf nextLeaf parent)=do
        ...
        let Just chunk=FS.compact $ B.concat $ map B.pack ([magic, wSize, wPrev, wNext]++value)
        FS.writePage file index chunk
    ...
    readBTree TableIndex {file=file, capacity=capacity} page parent=do
        ...
        if(head iter'==0) then do --BTreeNode
            ...
            let val_data=take (fromIntegral size) $ LR.chunksOf' (recordSize capacity) data_iter
    ...
    halfSplit :: [a]->([a],[a])
    halfSplit l=let (a, b)=splitAt ((length l) `div` 2) l in seq b (a, b)
